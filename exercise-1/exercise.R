...
response.country.code <- GET("http://data.unhcr.org/api/countries/list.json")
body <- fromJSON(content(response.country.code, "text"))
print(body)

body %>%  filter(name_en == "Syrian Arab Republic") %>%  select(country_code)
...
query.param <- list(year = 2013, country_of_origin = "SYR", country_of_residence = "USA")
response <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json", query = query.param)
body <- fromJSON(content(response, "text"))
...
query.param <- list(population_type_code = "RF", country_of_origin = "SYR", country_of_residence = "USA")
response <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json", query = query.param)
body <- fromJSON(content(response, "text"))
...
query.param <- list(population_type_code = "RF", country_of_origin = "SYR", country_of_residence = "TUR")
response <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json", query = query.param)
body <- fromJSON(content(response, "text"))
